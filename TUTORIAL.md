...
```python
s("/ms/ping")          # ->  <- /ms/pong []
s("/ms/version")       # ->  <- /ms/reply ['version', '0.17.0']
s("/ms/info")          # ->  <- /ms/reply ['info', 'musicscene', '0.17.0', ...]
```
...
- **Pattern-library trio (SuperCollider):** `examples/supercollider/example_two_hands_patterns.scd`
  — a pad melody over a two-hand piano, generated with SuperCollider Patterns (`Pbrown` melodic walks,
  `Prand` bar rhythms, `Pwrand` rests, `Pbind`/`Ppar` playback) instead of routines, shown live as a
  three-staff score (melody + braced piano) in D-flat natural minor (written as its enharmonic C-sharp
  minor), with a following playhead `cursor`.
- **Panola score — the simple way (SuperCollider):** `examples/supercollider/example_panola_score.scd`
  ...
  tuplets, auto-tied barline crossings, forced page/system breaks (`pageBreaks:` / `systemBreaks:`), plus
  per-note dynamics, articulations (combine several on one note with `+`, e.g. `@art^staccato+accent^`),
  slurs, and hairpins (`@hairpin^cresc^`) — no hand-built MEI. `MSScore` can also display a chosen page
  without playing (`showPage(n)` / `page(n)` / `nextPage` / `prevPage`). It is a separate quark:
