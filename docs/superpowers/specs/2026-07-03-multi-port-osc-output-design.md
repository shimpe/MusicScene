...
Only one process can `bind()` a given unicast UDP port. MusicScene sends every reply and event to a
...
This change lets MusicScene fan its outbound OSC out to a **configurable list of ports**, so a client and
...
- Configure the list two ways: a static `network/send_ports` project setting and the runtime
  `/ms/app output <host> <port> [port2 …]` command.
- Report the active output ports in the `/ms/info` reply.
...
| `nodes/MSRoot.gd` | Read new `network/send_ports` (String) alongside `network/send_port` (int); build the startup list; pass it to `server.start(...)`. |
| `core/OscDispatcher.gd` | `/ms/app output <host> <port…>` collects all trailing ports; append output ports to the `/ms/info` reply. |
...
- `get_send_ports() -> PackedInt32Array` (for `/ms/info` and tests)
...
Extend the existing `output` verb under `/ms/app`:
...
"output":
    ...
    if ports.is_empty():
        ctx.error("bad_arguments", "/ms/app", "output needs at least one port")
    else:
        ctx.server.set_output(host, ports)
```

`/ms/app output 127.0.0.1 7401` = today's behavior; `… 7401 7402 7403` fans out to three ports.
A missing port (`output <host>` with nothing after) is a `bad_arguments` error and leaves the current
list unchanged.

Append the active ports to `/ms/info` so a port conflict is diagnosable at a glance:

```gdscript
func _handle_info() -> void:
    var out := ["MusicScene", "0.11.0", "listen", ctx.server.get_listen_port(),
