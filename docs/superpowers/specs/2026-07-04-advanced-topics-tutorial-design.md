...
grammar of MusicScene's more advanced features, but not their *mechanics* or edge cases. Several genuinely
...
- Every example is a **Python `s(...)` call**, matching `TUTORIAL.md`, with expected replies/events shown
  as inline comments: `# -> /ms/reply <topic> …` or `# -> <address> <args…>`.
...
`/ms/scene/<id>/annotation <aid> text "<str>" | rect <x> <y> <w> <h> | glyph <name> | color <r> <g> <b> [a] | show | hide | del`,
and query `/ms/scene/<id>/annotations`. Cover: auto-creation on first reference to an unseen `<aid>`
...
the page automatically. Reply: `/ms/reply annotations <id> <aid…>` (ids only — no text/rect echoed).
...
engraved music. `/ms/scene/<id> addressable 1` **before** loading a source makes MusicScene generate
...
per node**: `/ms/reply discover <suggested_id> <path> <class> <name>`. `suggested_id` = the node's
...
reply `/ms/reply bindGroup <sub_id…>`. `bindAll meta <key> [value]` binds every scene node whose
metadata matches, via the normal permission-checked `bind()`. Also: **auto-bind at startup** — two frames
after ready, MusicScene binds every `OscExposable` node (with `osc_auto_bind and osc_allow_bind`) and every
...
**5. Safety & the permission model.** The capability gate protecting *pre-existing* project nodes (MusicScene's
...
`/ms/app/developer <0|1>`, `/ms/assets/allowScene "<path>"`, `/ms/assets/allowPrefix "<prefix>"`,
`/ms/assets/listAllowed`. Include a compact **gated-vs-not table** (bind/bindRel, bindGroup, bindAll,
...
touching the registration. `/ms/scene/<id>/payload <event> <token…>`. Token vocabulary (case-insensitive):
...
`/ms/event/physics <event> <self> <other> <intensity> <x> <y> <vx> <vy>` emitted regardless of bindings.
...
`/ms/scene/<id>/signal <godot_signal> <target> [payload <token…>]` escape hatch that connects *any* signal
