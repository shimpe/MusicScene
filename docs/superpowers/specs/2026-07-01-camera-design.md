# gscore_osc — OSC Camera Control (3D)
...
move or configure it over OSC. Add a `/gscore/camera` namespace so clients can frame the score-world:

```
s("/gscore/camera", "pos", 0.0, 0.0, 1.2)
s("/gscore/camera", "lookAt", 0.0, 0.0, 0.0)
s("/gscore/camera", "fov", 50)
s("/gscore/camera", "target", "note")      # keep 'note' centred as it moves
s("/gscore/camera", "follow", "note", 0.6) # chase-cam at distance 0.6
s("/gscore/camera", "projection", "orthographic")
s("/gscore/camera", "reset")
```
...
- **One dedicated subsystem.** A new `GScoreCamera` (`ctx.camera`) owns all camera state and control,
  ...
  (gscore-created or scene-provided).
...
- **Per-frame tracking via a small state machine.** `none` / `target` / `follow`, updated once per
  frame from `GScoreRoot._process`.

---

## 3. Architecture

**New file** `addons/gscore_osc/core/GScoreCamera.gd` (RefCounted). State:
...
```
`handle(rest, args)` accepts both the path-tail form (`/gscore/camera/pos …`) and the OSC-arg form
(`/gscore/camera` + `["pos", …]`): the verb is `rest[0]` if present else `args[0]`; the remaining
values are the params.

**GScoreRoot.** `var camera = null`; construct `camera = GScoreCamera.new(self)` (after `spatial`);
in `_process(delta)` call `camera.step(delta)`.

**GScoreSpatial3D.** Extract the default-camera framing (currently inline in `ensure_camera`) into a
...
perspective, identity rotation. `ensure_camera` calls it; `GScoreCamera` reset calls it via
