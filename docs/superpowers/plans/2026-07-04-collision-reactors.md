...
**Goal:** Add two Area-based collision reactors to gscore_osc — **bouncers** (mirror-reflect a colliding body's velocity + add an outward impulse) and **portals** (teleport a colliding body to a random linked partner) — working in both 2D and 3D, plus a rich generative pinball SuperCollider example.

**Architecture:** Reactors are `Area2D/3D` sensor objects (like zones). A new manager `GScoreReactors` stores per-object config and does the reflect/teleport work; it is invoked from the existing collision hook `GScorePhysicsAdapter._on_area_enter`, dispatching on `GScoreObject.type_hint`. All geometry/actuation is done in **world space** through `ctx.spatial` (dimension-agnostic).
...
- Tests must pass in **both** spaces. Default `gscore_osc/space` is `3d`. To run a test in 2D, temporarily set the project setting, OR (preferred, no file mutation) the test itself is written space-agnostically and is run once as-is (3d) in CI; where a task's test is space-sensitive it is noted.
...
| File | Responsibility |
|---|---|
| `addons/gscore_osc/physics/GScoreReactors.gd` (new) | The reactor manager: per-id config (bouncer params, portal links), `configure_bouncer`/`configure_portal`, `on_contact` dispatch, `_bounce`, `_teleport`, portal cooldown. |
| `addons/gscore_osc/core/GScoreSpatial2D.gd` / `GScoreSpatial3D.gd` | Add `body_set_velocity_world`, `is_dynamic`, `reactor_normal`, and `"bouncer"`/`"portal"` cases in `create_primitive`. |
| `addons/gscore_osc/core/GScoreFactory.gd` | Register `bouncer`/`portal` in `BUILTIN_TYPES`. |
| `addons/gscore_osc/core/GScoreRegistry.gd` | Auto-enable the area adapter for a newly-created bouncer/portal. |
| `addons/gscore_osc/physics/GScorePhysicsAdapter.gd` | Call `ctx.reactors.on_contact(obj, other)` from `_on_area_enter`. |
| `addons/gscore_osc/core/OscDispatcher.gd` | Route `/gscore/scene/<id>/bouncer` and `/portal` subsystems; bump version. |
| `addons/gscore_osc/nodes/GScoreRoot.gd` | Construct `ctx.reactors`. |
...
**Files:**
- Modify: `addons/gscore_osc/core/GScoreFactory.gd` (BUILTIN_TYPES)
- Modify: `addons/gscore_osc/core/GScoreSpatial2D.gd` (`create_primitive`)
- Modify: `addons/gscore_osc/core/GScoreSpatial3D.gd` (`create_primitive`)
- Modify: `addons/gscore_osc/core/GScoreRegistry.gd` (`create_builtin`)
- Modify: `addons/gscore_osc/physics/GScorePhysicsWorld.gd` (add a public `enable_area(obj)` helper)
...
func _process(_d: float) -> bool:
	...
	return false
```
(First confirm the exact accessors: `osc.registry.get_object(id)`, `obj.physics_adapter`, `obj.type_hint`, `osc.ctx.spatial.is_area(body)`, and how the autoload exposes `ctx`/`registry`/`dispatcher` — read `nodes/GScoreRoot.gd` and `tools/test_zones.gd` which already uses `osc.registry.get_object(...)` and `.physics_adapter.body`. Adjust accessor names to match if they differ, e.g. `osc.ctx` vs `osc` for spatial.)

- [ ] **Step 2: Run both tests, confirm they FAIL** (unknown type `bouncer`/`portal` → object not created).

- [ ] **Step 3: Register the types.** In `addons/gscore_osc/core/GScoreFactory.gd`, add `"bouncer", "portal"` to `BUILTIN_TYPES`.

- [ ] **Step 4: Add the primitive cases.** In BOTH `GScoreSpatial2D.gd` and `GScoreSpatial3D.gd`, inside `create_primitive`, add cases that build the SAME node as the existing `"area"` case:
```gdscript
		"bouncer", "portal":
			return _make_area_primitive(args)   # whatever the "area" case does; clone it
```
Read the existing `"area"` branch (`GScoreSpatial2D.gd` ~lines 224-229, `GScoreSpatial3D.gd` ~lines 375-380) and reproduce its node construction for these two type names (an `Area2D`/`Area3D` with a default `CollisionShape` + a small visual, exactly as `"area"` does). If the area branch is a couple of inline lines, just add `"bouncer", "portal"` to that same case's match list.

- [ ] **Step 5: Auto-enable the area adapter on creation.** In `addons/gscore_osc/core/GScoreRegistry.create_builtin`, after `obj.type_hint = type`, add:
```gdscript
	if type == "bouncer" or type == "portal":
		ctx.physics_world.enable_area(obj)
```
Add the helper to `addons/gscore_osc/physics/GScorePhysicsWorld.gd`:
```gdscript
...
```
(Confirm `_ensure_adapter` and `enable("area")` names against `GScorePhysicsWorld.handle_object` / `GScorePhysicsAdapter.enable`; the `handle_object` "enable" case already calls this path.)
...
- [ ] **Step 7: Commit.**
```bash
git add addons/gscore_osc/core/GScoreFactory.gd addons/gscore_osc/core/GScoreSpatial2D.gd addons/gscore_osc/core/GScoreSpatial3D.gd addons/gscore_osc/core/GScoreRegistry.gd addons/gscore_osc/physics/GScorePhysicsWorld.gd tools/test_bouncers.gd tools/test_bouncers.gd.uid tools/test_portals.gd tools/test_portals.gd.uid
git commit -m "feat(reactors): new bouncer/portal Area object types"
```

---

## Task 2: GScoreReactors manager — config, wiring, dispatch hook
...
**Files:**
- Create: `addons/gscore_osc/physics/GScoreReactors.gd`
- Modify: `addons/gscore_osc/nodes/GScoreRoot.gd` (construct `ctx.reactors`)
- Modify: `addons/gscore_osc/core/OscDispatcher.gd` (`_handle_scene_subsystem`: `bouncer`/`portal` cases)
- Modify: `addons/gscore_osc/physics/GScorePhysicsAdapter.gd` (`_on_area_enter` hook)
- Modify: `tools/test_bouncers.gd`, `tools/test_portals.gd` (add config assertions)

- [ ] **Step 1: Write the manager (config + stub).** Create `addons/gscore_osc/physics/GScoreReactors.gd`:
```gdscript
extends RefCounted
...
- [ ] **Step 2: Add the `is_dynamic` spatial helper.** In `GScoreSpatial2D.gd` add `func is_dynamic(node: Node) -> bool: return node is RigidBody2D`; in `GScoreSpatial3D.gd` add `func is_dynamic(node: Node) -> bool: return node is RigidBody3D`.

- [ ] **Step 3: Construct the manager.** In `nodes/GScoreRoot.gd`, where the other managers are created (near `physics_world`, `joints`, `events`), add `ctx.reactors = GScoreReactors.new(ctx)` with a `const GScoreReactors := preload("res://addons/gscore_osc/physics/GScoreReactors.gd")` at the top (match the existing preload/manager-construction style — read how `physics_world`/`events` are constructed and assigned onto `ctx`).
...
- [ ] **Step 5: Fire the hook.** In `physics/GScorePhysicsAdapter.gd` `_on_area_enter(other)`, after the existing `CollisionEvents.emit(ctx, obj, "areaEnter", other)`, add:
```gdscript
...
- [ ] **Step 8: Commit.**
```bash
git add addons/gscore_osc/physics/GScoreReactors.gd addons/gscore_osc/core/GScoreSpatial2D.gd addons/gscore_osc/core/GScoreSpatial3D.gd addons/gscore_osc/nodes/GScoreRoot.gd addons/gscore_osc/core/OscDispatcher.gd addons/gscore_osc/physics/GScorePhysicsAdapter.gd tools/test_bouncers.gd tools/test_portals.gd
...
**Files:**
- Modify: `addons/gscore_osc/core/GScoreSpatial2D.gd` / `GScoreSpatial3D.gd` (`body_set_velocity_world`, `reactor_normal`)
- Modify: `addons/gscore_osc/physics/GScoreReactors.gd` (`_bounce`)
...
```gdscript
	if _f == 3:
		...
		osc.dispatcher.dispatch("/gscore/scene/bmp", ["pos", 0.5, 0.5, 0.0])
		osc.dispatcher.dispatch("/gscore/scene/bmp/bouncer", ["strength", 2.0, "gain", 1.0])
		...
		osc.dispatcher.dispatch("/gscore/scene/ball", ["pos", 0.34, 0.5, 0.0])
...
- [ ] **Step 3: Add the world-space velocity setter + normal helper.** In `GScoreSpatial2D.gd`:
```gdscript
...
func _reactor_is_box(reactor: Node) -> bool:
	...
	return false
```
In `GScoreSpatial3D.gd` add the 3D equivalents (`RigidBody3D.linear_velocity`; `reactor_normal` using `Vector3` and the node's `global_transform.basis` columns x/y/z as the three axes; `_reactor_is_box` checks `CollisionShape3D` + `BoxShape3D`):
```gdscript
func body_set_velocity_world(body: Node, v) -> void:
...
- [ ] **Step 4: Implement `_bounce`.** Replace the stub in `GScoreReactors.gd`:
```gdscript
...
```
(`obj.node` is the reactor's Godot node — confirm the accessor name on `GScoreObject`; it may be `obj.node`. `v`, `n`, `v_out` are `Vector2` in 2D and `Vector3` in 3D; the arithmetic is identical for both, so no per-dimension branching here.)
...
- [ ] **Step 7: Commit.**
```bash
git add addons/gscore_osc/core/GScoreSpatial2D.gd addons/gscore_osc/core/GScoreSpatial3D.gd addons/gscore_osc/physics/GScoreReactors.gd tools/test_bouncers.gd
...
**Files:**
- Modify: `addons/gscore_osc/physics/GScoreReactors.gd` (`_teleport`)
...
```gdscript
	if _f == 3:
		osc.dispatcher.dispatch("/gscore/scene/pa", ["new", "portal"])
		osc.dispatcher.dispatch("/gscore/scene/pa/collider", ["circle", 0.1])
		osc.dispatcher.dispatch("/gscore/scene/pa", ["pos", 0.3, 0.5, 0.0])
		osc.dispatcher.dispatch("/gscore/scene/pb", ["new", "portal"])
		osc.dispatcher.dispatch("/gscore/scene/pb/collider", ["circle", 0.1])
		osc.dispatcher.dispatch("/gscore/scene/pb", ["pos", 0.8, 0.5, 0.0])
		osc.dispatcher.dispatch("/gscore/scene/pa/portal", ["link", "pb"])
		osc.dispatcher.dispatch("/gscore/scene/ball", ["new", "sphere", 0.03])
		osc.dispatcher.dispatch("/gscore/scene/ball/physics", ["enable", "rigid"])
		osc.dispatcher.dispatch("/gscore/scene/ball/collider", ["sphere", 0.03])
		osc.dispatcher.dispatch("/gscore/scene/ball", ["pos", 0.2, 0.5, 0.0])
		osc.dispatcher.dispatch("/gscore/scene/ball/physics", ["velocity", 0.5, 0.0, 0.0])   # heads into A
	if _f == 40:
		var b = osc.registry.get_object("ball")
...
- [ ] **Step 3: Implement `_teleport`.** Replace the stub in `GScoreReactors.gd`:
```gdscript
...
```
(Confirm `t.node`/`obj.node` accessors and `ctx.mapper.physics_mode` — the latter is used in `GScoreCollisionEvents._build_data`. `point_to_norm`/`vector_to_norm` return `Vector3` in both dimensions; `set_position`'s z arg is ignored in 2D.)
...
- [ ] **Step 6: Commit.**
```bash
git add addons/gscore_osc/physics/GScoreReactors.gd tools/test_portals.gd
...
**Files:**
- Modify: `addons/gscore_osc/core/OscDispatcher.gd` (3× `0.11.0` → `0.12.0`)
- Modify: `addons/gscore_osc/plugin.cfg` (`version="0.12.0"`)
- Modify: `README.md`, `TUTORIAL.md`, `ADVANCED.md`, `CHANGELOG.md`

- [ ] **Step 1: Bump version.** In `OscDispatcher.gd` replace all three `0.11.0` with `0.12.0`; in `plugin.cfg` set `version="0.12.0"`. Confirm: `grep -n '0\.11\.0' addons/gscore_osc/core/OscDispatcher.gd` returns nothing; `grep -c '0\.12\.0'` returns 3.

- [ ] **Step 2: README.** Add `bouncer` and `portal` to the object-types list and a short reference block for the new commands:
```
/gscore/scene/<id> new bouncer            # Area that mirror-reflects + kicks a body that enters
/gscore/scene/<id>/bouncer strength <s> gain <g> minSpeed <m>
/gscore/scene/<id> new portal             # Area that teleports an entering body to a random linked portal
/gscore/scene/<id>/portal link <id...>    # directional targets;  portal unlink to clear
```
...
s("/gscore/scene/p1/portal", "link", "p2")
```
...
```bash
git add addons/gscore_osc/core/OscDispatcher.gd addons/gscore_osc/plugin.cfg README.md TUTORIAL.md ADVANCED.md CHANGELOG.md
