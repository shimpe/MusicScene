...
**Goal:** Let MusicScene fan its outbound OSC (replies + events) to a configurable list of ports (default stays the single port 7401), so a client and monitors can each receive a copy.

**Architecture:** All outbound OSC funnels through `OscServer._send_bytes()`; change the scalar `_send_port` to a `_send_ports: PackedInt32Array` and loop it there. Configure the list via a new `network/send_ports` project setting (parsed at startup) and by extending the runtime `/ms/app/output <host> <port…>` command. Default (empty setting, single-port command) is byte-for-byte identical to today.
...
**Files:**
- Modify: `addons/musicscene/core/OscServer.gd`
- Modify: `addons/musicscene/nodes/MSRoot.gd` (call-site signature fix only)
- Modify: `addons/musicscene/core/OscDispatcher.gd` (call-site signature fix only)
...
In `addons/musicscene/core/OscServer.gd`:
...
func start(listen_port: int, send_host: String, send_ports: PackedInt32Array) -> bool:
	...
	if err != OK:
		push_error("[MusicSceneOSC] Failed to bind UDP port %d (error %d)" % [_listen_port, err])
		_running = false
		return false
	_running = true
	if verbose:
		print("[MusicSceneOSC] OSC server listening on udp:%d, replies -> %s:%s"
...
func set_output(host: String, send_ports: PackedInt32Array) -> void:
	...
	_send_ports = _normalize_ports(send_ports)
	if verbose:
		print("[MusicSceneOSC] OSC output set to %s:%s" % [host, str(Array(_send_ports))])
```
...
In `addons/musicscene/nodes/MSRoot.gd`, change the third argument of the `server.start(...)` call from:
```gdscript
...
In `addons/musicscene/core/OscDispatcher.gd`, change the `"output":` case from:
```gdscript
		"output":
...
```
Confirm `tools/test_osc_output.gd.uid` now exists. Then run the test (see Conventions). Expected final line: `DONE pass=12 fail=0`, and no `FAIL:` line. (9 unit + 2 setup + 1 happy-path fan-out check; the `_f >= 120` timeout branch only runs — and only fails — if a receiver never got the packet, so a clean run is 12, not 14.)

- [ ] **Step 6: Commit**

```bash
git add addons/musicscene/core/OscServer.gd addons/musicscene/nodes/MSRoot.gd addons/musicscene/core/OscDispatcher.gd tools/test_osc_output.gd tools/test_osc_output.gd.uid
...
**Files:**
- Modify: `addons/musicscene/nodes/MSRoot.gd`

- [ ] **Step 1: Replace the autostart block**

In `addons/musicscene/nodes/MSRoot.gd`, replace the whole autostart block that Task 1 left as:
```gdscript
...
```gdscript
	if bool(_setting("network/autostart", true)):
		...
		server.start(
			...
			send_ports)
```
(`OscServer` is already preloaded as a const at the top of `MSRoot.gd`, so the static call resolves.)
...
```bash
git add addons/musicscene/nodes/MSRoot.gd
git commit -m "feat(osc): read network/send_ports at startup (empty -> single default)"
```

---

### Task 3: Runtime command — `/ms/app/output <host> <port…>` + `/ms/info` ports

Extends the existing `output` app command to accept any number of ports, and exposes the active ports in the `/ms/info` reply via a small testable `_info_payload()` seam.
...
In `addons/musicscene/core/OscDispatcher.gd`, replace the `"output":` case (the single-port version Task 1 left) with:
```gdscript
			"output":
				...
				if out_ports.is_empty():
					ctx.error("bad_arguments", "/ms/app/output", "output needs at least one port")
				else:
					ctx.server.set_output(out_host, out_ports)
```

- [ ] **Step 4: Implement the `/ms/info` port list**
...
```bash
git add addons/musicscene/core/OscDispatcher.gd tools/test_osc_output_cmd.gd tools/test_osc_output_cmd.gd.uid
git commit -m "feat(osc): /ms/app/output takes a port list; /ms/info reports it"
```
...
In `addons/musicscene/core/OscDispatcher.gd`, replace `"0.10.0"` with `"0.11.0"` in all 3 occurrences (the `version` reply in `dispatch`, the `version` reply in `_handle_root`, and the `"MusicScene", "0.10.0"` pair now inside `_info_payload`). Verify: `grep -n '0\.10\.0' addons/musicscene/core/OscDispatcher.gd` returns nothing; `grep -n '0\.11\.0' addons/musicscene/core/OscDispatcher.gd` returns 3 lines.

- [ ] **Step 2: Bump `plugin.cfg`**

In `addons/musicscene/plugin.cfg`, change:
```
version="0.10.0"
```
to:
...
In the network/ports section (near where `network/send_port` / the reply port is documented — find it with `grep -n "send_port\|7401\|reply port\|/ms/app" README.md`), add:
```markdown
### Multiple output ports

By default MusicScene sends every reply and event to a single port (`network/send_port`, default 7401),
...
`/ms/info` reports the active list (… `output 7401 7402`). With `network/send_ports` unset,
...
```bash
git add addons/musicscene/core/OscDispatcher.gd addons/musicscene/plugin.cfg README.md TUTORIAL.md CHANGELOG.md
...
- Runtime `/ms/app/output <host> <port…>`, zero ports = error → Task 3. ✓
- `/ms/info` reports ports → Task 3 (`_info_payload`, tested). ✓
