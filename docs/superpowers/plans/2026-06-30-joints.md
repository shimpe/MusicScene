...
**Goal:** Add OSC-controlled physics joints (`/gscore/joint/<id>`) to gscore_osc in both 2D and 3D, exposing each engine's native joint types.

**Architecture:** Mirror the existing physics trio. A dimension-agnostic `GScoreJointWorld` (router + per-frame breakForce monitor, like `GScorePhysicsWorld`) owns an id→`GScoreJoint` map. Each `GScoreJoint` (like `GScorePhysicsAdapter`) wraps one Godot joint node plus its two endpoint objects and cached params. All native joint creation and parameter mutation lives in the spatial backends (`GScoreSpatial2D` / `GScoreSpatial3D`), selected by `gscore_osc/space`.

**Tech Stack:** Godot 4.7, GDScript. Tests are headless SceneTree scripts run via `godot --headless --path . --script res://tools/<t>.gd`, asserting through the live `GScoreOSC` autoload and printing `PASS:`/`FAIL:` lines (same style as `tools/test_recreate.gd`).
...
- **ctx (the autoload `GScoreRoot`)** exposes: `ctx.spatial`, `ctx.registry`, `ctx.physics_world`, `ctx.objects_root`, `ctx.mapper`, `ctx.verbose`, and helpers `ctx.reply(topic, values)`, `ctx.error(code, address, msg)`, `ctx.send_event(address, args)`. Physics coord mode is `ctx.mapper.physics_mode`.
...
|---|---|---|
| `addons/gscore_osc/physics/GScoreJointWorld.gd` | New | Router for `/gscore/joint(s)`, id→joint map, per-frame break monitor + prune, node placement. |
| `addons/gscore_osc/physics/GScoreJoint.gd` | New | One joint: Godot node + endpoints + params + active-DOF cursor + break logic. |
| `addons/gscore_osc/core/GScoreSpatial2D.gd` | Mod | `joint_types`, `make_joint`, `joint_attach`, `joint_set_param`, `joint_separation`, `to_native_length`. |
| `addons/gscore_osc/core/GScoreSpatial3D.gd` | Mod | Same method set, native 3D joints incl. generic6dof per-DOF. |
| `addons/gscore_osc/core/OscDispatcher.gd` | Mod | Route `joint` / `joints`. |
| `addons/gscore_osc/nodes/GScoreRoot.gd` | Mod | Construct `ctx.joints`; call `joints.physics_step(delta)`. |
| `tools/test_joints.gd` | New | Space-aware headless integration test. |
| `TUTORIAL.md`, `CHANGELOG.md`, `addons/gscore_osc/plugin.cfg`, `.github/workflows/ci.yml` | Mod | Docs, version bump, CI run. |
...
**Files:**
- Create: `addons/gscore_osc/physics/GScoreJointWorld.gd`
- Create: `addons/gscore_osc/physics/GScoreJoint.gd`
- Modify: `addons/gscore_osc/core/OscDispatcher.gd` (add `"joint"`/`"joints"` case)
- Modify: `addons/gscore_osc/nodes/GScoreRoot.gd` (declare `var joints`, construct it, step it)
...
- [ ] **Step 3: Create `GScoreJoint.gd` (minimal)**
...
- [ ] **Step 4: Create `GScoreJointWorld.gd` (skeleton)**
...
- [ ] **Step 6: Wire ctx in `GScoreRoot.gd`** — add the preload near the other physics preload, declare the member, construct it, and step it.
...
```
Construct (after `physics_world = GScorePhysicsWorld.new(self)`):
```gdscript
...
```bash
git add addons/gscore_osc/physics/GScoreJointWorld.gd addons/gscore_osc/physics/GScoreJoint.gd addons/gscore_osc/core/OscDispatcher.gd addons/gscore_osc/nodes/GScoreRoot.gd tools/test_joints.gd
...
This task adds backend joint primitives to **both** `GScoreSpatial2D` and `GScoreSpatial3D`, and wires `_create`. The live test exercises the active backend (3D by default); 2D is verified in Task 6.

**Files:**
- Modify: `addons/gscore_osc/core/GScoreSpatial2D.gd`
- Modify: `addons/gscore_osc/core/GScoreSpatial3D.gd`
- Modify: `addons/gscore_osc/physics/GScoreJointWorld.gd`
- Test: `tools/test_joints.gd`
...
- [ ] **Step 3: Add joint primitives to `GScoreSpatial2D.gd`** (append in the Physics section):
...
- [ ] **Step 4: Add joint primitives to `GScoreSpatial3D.gd`** (append in its Physics section). For 3D, attach places the joint at body A and orients its basis so local X points A→B (the natural axis for slider/hinge):
...
> `length_to_world(norm, mode)` already exists in `GScoreSpatial3D` (used by `make_collider`); confirm the signature and reuse it.

- [ ] **Step 5: Implement `_create` in `GScoreJointWorld.gd`** (replace the stub):
...
```bash
git add addons/gscore_osc/core/GScoreSpatial2D.gd addons/gscore_osc/core/GScoreSpatial3D.gd addons/gscore_osc/physics/GScoreJointWorld.gd tools/test_joints.gd
...
**Files:**
- Modify: `addons/gscore_osc/physics/GScoreJoint.gd` (add `apply` + setters)
- Modify: `addons/gscore_osc/physics/GScoreJointWorld.gd` (`handle` routes verbs to `j.apply`)
- Modify: `addons/gscore_osc/core/GScoreSpatial2D.gd` (`joint_set_param`)
- Modify: `addons/gscore_osc/core/GScoreSpatial3D.gd` (`joint_set_param`, incl. generic6dof DOF)
- Test: `tools/test_joints.gd`
...
```gdscript
	...
	if _f == 16:
		...
		if osc.spatial.is_3d():
			...
			check(absf((jn.node as DampedSpringJoint2D).stiffness - 0.7 * GScoreSpatial2DStiffMax()) < 1.0, "2D stiffness mapped")
```

> If referencing `GScoreSpatial2D.JOINT_STIFF_MAX` from the test is awkward, assert `(jn.node as DampedSpringJoint2D).stiffness > 50.0` instead (0.7 → ~105). Use whichever compiles cleanly; drop the helper call.
...
- [ ] **Step 3: Add `apply` + setters to `GScoreJoint.gd`**:
...
- [ ] **Step 4: Route property verbs in `GScoreJointWorld.handle`** — replace the `match verb:` block's `_:` arm:
...
- [ ] **Step 5: Implement `joint_set_param` in `GScoreSpatial2D.gd`**:
...
> `_pf(args, i, def)` already exists in `GScoreSpatial2D` (used by colliders) — reuse it.

- [ ] **Step 6: Implement `joint_set_param` in `GScoreSpatial3D.gd`** — native per-type, with generic6dof routed by `active_dof`:
...
```bash
git add addons/gscore_osc/physics/GScoreJoint.gd addons/gscore_osc/physics/GScoreJointWorld.gd addons/gscore_osc/core/GScoreSpatial2D.gd addons/gscore_osc/core/GScoreSpatial3D.gd tools/test_joints.gd
...
**Files:**
- Modify: `addons/gscore_osc/physics/GScoreJoint.gd` (`should_break`)
- Modify: `addons/gscore_osc/physics/GScoreJointWorld.gd` (`physics_step` break + event)
- Test: `tools/test_joints.gd`
...
- [ ] **Step 3: Add `should_break` to `GScoreJoint.gd`**:
...
- [ ] **Step 4: Break in `GScoreJointWorld.physics_step`** — replace the body:
...
```bash
git add addons/gscore_osc/physics/GScoreJoint.gd addons/gscore_osc/physics/GScoreJointWorld.gd tools/test_joints.gd
...
Godot reads an `override.cfg` at the project root to override project settings. Use it to flip the space to `2d` and run the same suite against `GScoreSpatial2D`, then remove it.
...
**Files:**
- Modify: `TUTORIAL.md`
- Modify/verify: `addons/gscore_osc/examples/ExamplePhysicalNote.tscn` is referenced by the tutorial
...
Visually skim the new section in `TUTORIAL.md`. Confirm the `ExamplePhysicalNote.tscn` reference (if you mention it) points to `res://addons/gscore_osc/examples/ExamplePhysicalNote.tscn`.
...
- [ ] **Step 2: Bump the addon version** in `addons/gscore_osc/plugin.cfg`:
...
```bash
git add CHANGELOG.md addons/gscore_osc/plugin.cfg addons/gscore_osc/core/OscDispatcher.gd .github/workflows/ci.yml
