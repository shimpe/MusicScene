extends SceneTree
## Headless test for `/gscore/scene/<id>/physics planar <0|1>` (pin a body to the z=0 plane).
...
func _process(_d: float) -> bool:
	...
	if _f == 2:
		...
		d.dispatch("/gscore/scene/b", ["new", "circle"])
		d.dispatch("/gscore/scene/b/physics", ["enable", "rigid"])
		d.dispatch("/gscore/scene/b", ["pos", 0.0, 0.5, 0.3])   # start off-plane (z != 0)
		d.dispatch("/gscore/physics", ["gravity", 0.0, -1.0, 0.0])
		d.dispatch("/gscore/physics", ["enable", 1])
	elif _f == 4:
		var body = osc.registry.get_object("b").node
		if osc.space == "3d":
			d.dispatch("/gscore/scene/b/physics", ["planar", 1])
			check(body.axis_lock_linear_z, "planar 1 locks the linear z axis")
		else:
			d.dispatch("/gscore/scene/b/physics", ["planar", 1])  # no-op, must not error
		...
		if osc.space == "3d":
			...
			d.dispatch("/gscore/scene/b/physics", ["planar", 0])
