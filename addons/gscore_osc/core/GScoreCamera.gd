extends RefCounted
## Controls the active 3D Camera3D over OSC (/gscore/camera ...). 3D only; in 2D every command errors.
## Holds per-frame tracking state (target/follow); GScoreRoot._process calls step(delta).
...
func handle(rest, args: Array) -> void:
	if not ctx.spatial.is_3d():
		ctx.error("bad_arguments", "/gscore/camera", "camera control is only available in 3d space")
		return
	var verb: String
	var p: Array
	if rest.size() > 0:
	...
	var cam = _cam()
	if cam == null:
		ctx.error("internal_error", "/gscore/camera", "no active camera"); return
	...
	match verb:
		...
		"target":
			...
			if ctx.registry.get_object(candidate) == null:
				ctx.error("unknown_object", "/gscore/camera", "Unknown target: " + candidate); return
		...
		_:
			ctx.error("bad_arguments", "/gscore/camera", "Unknown camera cmd: " + verb)
...
func _start_follow(cam, p: Array, mode: String) -> void:
	...
	if obj == null or not obj.is_valid():
		ctx.error("unknown_object", "/gscore/camera", "Unknown follow target: " + candidate); return
	_target_id = candidate
	_mode = "follow"
