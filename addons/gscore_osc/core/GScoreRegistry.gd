extends RefCounted
## The heart of OSC<->Godot identity decoupling. Maps stable OSC ids to GScoreObject wrappers,
...
const GScoreObject := preload("res://addons/gscore_osc/core/GScoreObject.gd")
const GScoreFactory := preload("res://addons/gscore_osc/core/GScoreFactory.gd")
const OscExposableClass := preload("res://addons/gscore_osc/nodes/OscExposable.gd")
...
var ctx = null
var _objects: Dictionary = {}        # id -> GScoreObject
...
func register_object(id: String, node: Node, ownership: String) -> GScoreObject:
	if node == null:
...
func create_builtin(id: String, type: String, args: Array) -> GScoreObject:
	var node := GScoreFactory.create(type, args, ctx)
	if node == null:
		return null
	...
	if ctx.verbose:
		print("[GScoreOSC] created %s '%s'" % [type, id])
	return obj


func bind(id: String, abs_path: String) -> GScoreObject:
	var node: Node = ctx.get_node_or_null(NodePath(abs_path))
	return _bind_node(id, node, abs_path, OWN_BOUND)


func bind_rel(id: String, rel_path: String) -> GScoreObject:
	var root := _resolve_app_root()
	if root == null:
		ctx.error("unknown_object", "/gscore/bindRel", "No app root set; use /gscore/app/root")
		return null
	var node: Node = root.get_node_or_null(NodePath(rel_path))
	return _bind_node(id, node, rel_path, OWN_BOUND)


func _bind_node(id: String, node: Node, where: String, ownership: String) -> GScoreObject:
	if node == null:
		ctx.error("unknown_object", "/gscore/bind", "No node at: " + where)
		return null
...
func instantiate(id: String, scene_path: String, parent_ref: String = "") -> GScoreObject:
	if not ctx.permissions.can_instantiate_path(scene_path):
		ctx.error("permission_denied", "/gscore/scene/" + id,
			"Scene not whitelisted: " + scene_path)
		return null
	if not ResourceLoader.exists(scene_path):
		ctx.error("load_failed", "/gscore/scene/" + id, "Scene not found: " + scene_path)
		return null
	var packed = load(scene_path)
	if not (packed is PackedScene):
		ctx.error("load_failed", "/gscore/scene/" + id, "Not a PackedScene: " + scene_path)
		return null
	...
	apply_exposure(obj)
	if ctx.verbose:
		print("[GScoreOSC] instantiated '%s' from %s" % [id, scene_path])
	return obj
...
func _cleanup(obj) -> void:
	...
	# Break the GScoreObject <-> adapter reference cycle so nothing leaks at exit.
...
func auto_bind_exposed() -> int:
	...
	if ctx.verbose and count > 0:
		print("[GScoreOSC] auto-bound %d exposed node(s)" % count)
	return count
