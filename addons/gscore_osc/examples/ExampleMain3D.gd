...
## without an external OSC client. Requires gscore_osc/space = "3d".

@export var auto_run_demo: bool = true
@export var demo_script: String = "res://addons/gscore_osc/examples/example_score_3d.gscore"
...
func _ready() -> void:
	...
	if osc == null:
		push_warning("[ExampleMain3D] GScoreOSC autoload not found; is the plugin enabled?")
		return
	if not osc.spatial.is_3d():
		push_warning("[ExampleMain3D] gscore_osc/space is not '3d'; set it in Project Settings.")
