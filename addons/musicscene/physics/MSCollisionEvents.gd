...
## dictionary, applies the matching MSEventBinding (threshold, cooldown/rate, filters,
## payload), sends to the bound target, and always emits canonical /ms/event/physics.
...
static func emit(ctx, obj, event: String, other: Node) -> void:
	...
	ctx.send_event("/ms/event/physics", [
