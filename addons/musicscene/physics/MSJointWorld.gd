...
func handle_global(rest, args: Array) -> void:
	# /ms/joints list  — accept both path-tail form and OSC-arg form
...
func _create(id: String, args: Array) -> void:
	if id.is_empty():
		ctx.error("bad_arguments", "/ms/joint", "Missing joint id"); return
...
func physics_step(_delta: float) -> void:
	...
	# springs, etc. are visible. Shares the /ms/physics debug toggle with collision-shape display.
