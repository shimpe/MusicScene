...
## ctx.spatial (MSSpatial2D or MSSpatial3D). If the object's node is already a physics
...
## force from MSPhysicsWorld, so OSC gravity control is exact and freeze/pause are clean.

const CollisionEvents := preload("res://addons/musicscene/physics/MSCollisionEvents.gd")
...
func _base() -> String:
	return "/ms/scene/" + obj.osc_id + "/physics"
...
func enable(kind: String) -> void:
	...
	if collider == null:
		set_collider("auto", [])
	if ctx.verbose:
		print("[MusicSceneOSC] physics enabled (%s) on '%s'" % [kind.to_lower(), obj.osc_id])
...
func set_collider(kind: String, params: Array) -> void:
	...
	if cs == null:
		ctx.error("bad_arguments", "/ms/scene/" + obj.osc_id + "/collider", "Bad collider: " + kind)
		return
