...
const Adapter := preload("res://addons/musicscene/physics/MSPhysicsAdapter.gd")
...
var _adapters: Array = []                   # MSPhysicsAdapter
	...
	match cmd:
		...
		"coord":
			...
			if ctx.mapper.is_valid_mode(mode):
				...
				ctx.error("bad_arguments", "/ms/physics", "Bad coord mode: " + mode)
		"debug":
			...
			for a in _adapters:
				if a.is_valid():
					a.set_debug(debug)
		_:
			ctx.error("bad_arguments", "/ms/physics", "Unknown physics cmd: " + cmd)
...
func handle_object(obj, args: Array) -> void:
	...
	if obj.physics_adapter == null:
		ctx.error("bad_arguments", "/ms/scene/" + obj.osc_id + "/physics",
			"Enable physics first: /ms/scene/%s/physics enable <static|rigid|area>" % obj.osc_id)
		return
...
func handle_collider(obj, args: Array) -> void:
	...
	match cmd:
		...
		_:
			ctx.error("bad_arguments", "/ms/scene/" + obj.osc_id + "/collider",
