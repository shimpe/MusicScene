...
const NotationObject3D := preload("res://addons/musicscene/notation/MSNotationObject3D.gd")
...
func set_size(node: Node, w: float, h: float, mode: String) -> void:
	...
	if node.has_method("ms_set_size"):
		node.ms_set_size(ww, hw)
...
func get_size_norm(node: Node, mode: String) -> Vector2:
	...
	elif node.has_method("ms_get_world_size"):
		sz = node.ms_get_world_size()
	if mode == "normalized":
...
func set_opacity(node: Node, a: float) -> void:
	if node.has_method("ms_set_opacity"):
		node.ms_set_opacity(a)
...
func set_color(node: Node, c: Color) -> void:
	if node.has_method("ms_set_color"):
		node.ms_set_color(c)
	elif node is MeshInstance3D:
...
func set_global_shade_mode(mode: String) -> void:
	if not (mode in ["auto", "shaded", "flat"]):
		...
		return
	shade_mode = mode
	# Re-shade only MusicScene-created primitives; never overwrite a bound or instantiated
	...
	match type:
		...
		"image", "sprite":
			...
			if args.size() > 0:
				...
				if tex == null:
					ctx.error("load_failed", "/ms/scene", "Could not load image: " + str(args[0]))
				else:
					sp.texture = tex
			return sp
		"area", "bouncer", "portal":
		...
		"cone":
			...
			return mi
		_:
			ctx.error("bad_arguments", "/ms/scene", "Unknown built-in type: " + type)
			return null
