...
const Primitive := preload("res://addons/musicscene/core/MSPrimitive2D.gd")
const NotationObject := preload("res://addons/musicscene/notation/MSNotationObject.gd")
const Collider := preload("res://addons/musicscene/physics/MSColliderBuilder.gd")
const InputUtil := preload("res://addons/musicscene/events/MSInputEvents.gd")
...
func get_size_norm(node: Node, mode: String) -> Vector2:
	...
	elif node.has_method("ms_get_bounds"):
		px = node.ms_get_bounds().size
	elif node is Control:
