...
const MS_VERSION := "0.18.0"  # reported by /ms/version and /ms/info — keep in sync with plugin.cfg
...
const NOTATION_VERBS := [
	...
	"part", "notationinfo", "currentpage", "addressable", "paginate", "measures", "elements",
	"background", "bg",
]
...
func dispatch(address: String, args: Array) -> void:
	...
	if parts.is_empty() or parts[0] != "ms":
	...
	match head:
		...
		_:
			ctx.error("bad_arguments", address, "Unknown namespace: /ms/" + head)
	...
	match key:
		"coord":
			...
			if ctx.mapper.is_valid_mode(mode):
				...
				ctx.error("bad_arguments", "/ms/app/coord", "Bad coord mode: " + mode)
		"root":
		...
		"output":
			...
			if out_ports.is_empty():
				ctx.error("bad_arguments", "/ms/app/output", "output needs at least one valid port")
			else:
				ctx.server.set_output(out_host, out_ports)
		"developer", "developer_mode":
			ctx.permissions.developer_mode = _b(args, 0)
		_:
			ctx.error("bad_arguments", "/ms/app", "Unknown app key: " + key)
...
func _handle_permissions(args: Array) -> void:
	...
	match key:
		...
		_: ctx.error("bad_arguments", "/ms/app/permissions", "Unknown flag: " + key)
...
func _handle_assets(rest, args: Array) -> void:
	...
	match key:
		...
		_: ctx.error("bad_arguments", "/ms/assets", "Unknown assets key: " + key)
...
func _handle_notation_global(rest, args: Array) -> void:
	...
	if rest.size() > 0 and rest[0] == "cache":
		...
		ctx.error("bad_arguments", "/ms/notation", "Expected cache")
...
func _handle_script(rest, args: Array) -> void:
	...
	match key:
		...
		_: ctx.error("bad_arguments", "/ms/script", "Expected run|load")
...
func _handle_scene_subsystem(id: String, sub: String, args: Array) -> void:
	var obj = ctx.registry.get_object(id)
	if obj == null:
		ctx.error("unknown_object", "/ms/scene/" + id + "/" + sub, "Unknown object: " + id)
		return
	match sub:
		...
		_:
			ctx.error("bad_arguments", "/ms/scene/" + id + "/" + sub,
