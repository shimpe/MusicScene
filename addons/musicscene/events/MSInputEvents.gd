...
static func object_hit(obj, global_point: Vector2) -> bool:
	...
	if target == null:
		return false
	if target.has_method("ms_contains_point"):
		return target.ms_contains_point(target.to_local(global_point))
...
static func _visual_of(obj) -> Node:
	...
	if node == null:
		return null
	if node.has_method("ms_contains_point"):
		return node
	# Physics-wrapped: the drawing node is a child of the body.
	for c in node.get_children():
		if c.has_method("ms_contains_point"):
			return c
	return node
