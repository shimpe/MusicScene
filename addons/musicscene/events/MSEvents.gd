...
## MSEventBinding on the object (wired by the physics adapter); input events are stored as
## lightweight dicts.

const EventBinding := preload("res://addons/musicscene/events/MSEventBinding.gd")
const SignalBinding := preload("res://addons/musicscene/events/MSSignalBinding.gd")
...
func handle_on(obj, args: Array) -> void:
	...
	if event == "" or target == "":
		ctx.error("bad_arguments", "/ms/scene/" + obj.osc_id + "/on", "Need <event> <target>")
		return
...
func handle_signal(obj, args: Array) -> void:
	...
	var target := str(args[1]) if args.size() > 1 else ""
	if sig == "" or target == "":
		ctx.error("bad_arguments", "/ms/scene/" + obj.osc_id + "/signal", "Need <signal> <target>")
		return
...
func _emit_input(h: Dictionary, event: String) -> void:
	if h.type == "obj":
		...
		ctx.send_event("/ms/event/input", [event, obj.osc_id, h.nx, h.ny])
	elif h.type == "region":
		var obj = h.obj
		var reg = h.region
		if reg.bindings.has(event):
			ctx.send_event(reg.bindings[event], [obj.osc_id, reg.region_id, h.u, h.v])
		ctx.send_event("/ms/event/input", [event, obj.osc_id + "/" + reg.region_id, h.u, h.v])
...
func _update_hover(hits: Array) -> void:
	...
	for id in present.keys():
		if not _hover.get(id, false):
			...
			ctx.send_event("/ms/event/input", ["enter", obj.osc_id, h.nx, h.ny])
	# left
	for id in _hover.keys().duplicate():
		if _hover[id] and not present.has(id):
			...
			ctx.send_event("/ms/event/input", ["leave", id, 0.0, 0.0])
