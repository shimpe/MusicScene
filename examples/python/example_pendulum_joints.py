#!/usr/bin/env python3
"""
Double-pendulum JOINTS demo for MusicScene (3D space).
...
bottom of the swing; when the pendulum tip crosses one, MusicScene emits
...
def build():
    ...
    s("/ms/scene/post", "pos", 0.0, 0.7, 0.0)
    s("/ms/scene/post/collider", "disabled", 1)   # anchor only: don't bounce the tip off it

    s("/ms/scene/arm1", "new", "circle")
    s("/ms/scene/arm1/physics", "enable", "rigid")
    s("/ms/scene/arm1", "pos", 0.22, 0.7, 0.0)

    s("/ms/scene/arm2", "new", "circle")
    s("/ms/scene/arm2/physics", "enable", "rigid")
    s("/ms/scene/arm2", "pos", 0.44, 0.7, 0.0)     # both arms out to one side = lots of energy

    s("/ms/joint/h1", "new", "hinge", "post", "arm1")
    s("/ms/joint/h1", "axis", 0, 0, 1)
    s("/ms/joint/h2", "new", "hinge", "arm1", "arm2")
    s("/ms/joint/h2", "axis", 0, 0, 1)
    ...
    for pid, x, y, note in pads:
        s(f"/ms/scene/{pid}", "new", "rect")
        s(f"/ms/scene/{pid}/physics", "enable", "area")
        s(f"/ms/scene/{pid}/collider", "rect", 0.16, 0.14)
        s(f"/ms/scene/{pid}", "pos", x, y, 0.0)
        # only the tip triggers; a small cooldown avoids double-hits on a slow pass
        s(f"/ms/scene/{pid}/on", "areaEnter", "/music/note", "other", "arm2", "cooldown", 0.08)
        # payload:  <pad id>  <note (literal)>  <tip speed>
        s(f"/ms/scene/{pid}/payload", "areaEnter", "self", f"={note}", "otherspeed")

    s("/ms/physics", "debug", 1)          # show the hinges + pad zones (send "debug 0" to hide)
